...
qemu_option="-M virt -smp 4 -m 1024 -nographic -cpu cortex-a57"
qemu_setup_network=""
qemu_instance_id=""
img_copy_option="-n"

kernel_bootargs="console=ttyAMA0 init=/bin/init hardware=qemu.arm.linux root=/dev/ram0 rw ohos.required_mount.system=/dev/block/vdb@/usr@ext4@ro,barrier=1@wait,required ohos.required_mount.vendor=/dev/block/vdc@/vendor@ext4@ro,barrier=1@wait,required"
...
echo qemu-system-aarch64 ${qemu_option} -append "${kernel_bootargs}"
