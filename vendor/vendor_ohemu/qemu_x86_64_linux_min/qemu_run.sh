...
qemu_option="-machine microvm -smp 4 -m 1024 -nographic"
qemu_setup_network=""
qemu_instance_id=""
img_copy_option="-n"

kernel_bootargs="console=ttyS0,115200 init=/bin/init hardware=qemu.x86_64.linux default_boot_device=a003e00.virtio_mmio root=/dev/ram0 rw ohos.required_mount.system=/dev/block/vdb@/usr@ext4@ro,barrier=1@wait,required ohos.required_mount.vendor=/dev/block/vdc@/vendor@ext4@ro,barrier=1@wait,required"

elf_file="out/qemu-x86_64-linux/packages/phone/images"
...
function qemu_network()
{
    ...
    if [ $? -ne 0 ]; then
        qemu_install_path=`which qemu-system-x86_64`
...
qemu_option_add "-kernel $elf_file/bzImage -initrd $elf_file/ramdisk.img"
...
[ x"${qemu_setup_network}" != x ] && sudo qemu-system-x86_64 ${qemu_option} -append "${kernel_bootargs}"

# start without sudo if no need to setup network
[ x"${qemu_setup_network}" == x ] && qemu-system-x86_64 ${qemu_option} -append "${kernel_bootargs}"
